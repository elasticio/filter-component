 ...
 */
function init(cfg) {
    logger.info('Initialization started cfg=%j', cfg);
    if (!cfg || !cfg.reject) {
        logger.info('Initialization not required, no reject is specified');
        return;
    }
    return co(function* init() {
        const flowID = cfg.reject;
        logger.info('Fetching the flow id=%s', flowID);
        const flow = yield rp({
        ...
        logger.info('Fetched flow=%j', flow);
        ...
        if (attributes.status !== 'active') {
            logger.warn('WARNING: You configured reject flow but this flow '
        ...
        logger.info('Initialization completed rejectRoutingKey=%s', rejectRoutingKey);
    ...
    if (context.__eio_eval_result) {
        ...
        if (rejectRoutingKey) {
            ...
            rp({
                ...
                that.logger.info('Message sent to flow msg_id=%s flowID=%s response=%j',
...
function getFlows(cfg) {
    return co(function* gen() {
        ...
        logger.info('Fetching user flows cfg=%j', cfg);
        ...
        logger.info('Found %s flows', flows.data.length);
        const wWebhook = flows.data.filter((flow) => findWebhookStepID(flow.attributes.graph) !== null);
        logger.info('Found %s of my flows with webhooks', wWebhook.length);
        const result = {};
