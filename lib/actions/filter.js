...
function init(cfg) {
    ...
    return co(function* init() {
        ...
        rejectRoutingKey = `${userID}.${flowID}/${type}.${stepID}.message`;
...
function getFlows(cfg) {
    return co(function* gen() {
        ...
        const myFlows = flows.data
            ...
            .filter((flow) => findWebhookStepID(flow.attributes.graph) !== null);
        const result = {};
        myFlows.forEach((flow) => result[flow.id] = flow.attributes.name);
