 ...
 * @type {string}
 */
const HEADER_TASK_ID_KEY = 'X-EIO-TaskID-Key';
...
function processAction(msg, cfg) {
    ...
    if (context.__eio_eval_result) {
        ...
        if (rejectRoutingKey) {
            console.log('Your message will be rejected to flowID=%s', cfg.reject);
            msg.headers = msg.headers || {};
            msg.headers[HEADER_ROUTING_KEY] = rejectRoutingKey;
            msg.headers[HEADER_TASK_ID_KEY] = cfg.reject;
            console.log('Emitting message=%j', msg);
            this.emit('data', msg);
        }
    }
    this.emit('end');
