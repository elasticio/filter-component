...
function clearResponse(result, addMetadataToResponse) {
  if (result.elasticio) {
    ...
    if (addMetadataToResponse) resultCopy.elasticioMeta = resultCopy.elasticio;
    delete resultCopy.elasticio;
    return resultCopy;
  }
  return result;
}
...
exports.process = async function simpleJSONataFilter(msg, cfg) {
  ...
  if (result) {
    const clearBody = clearResponse(msg.body, cfg.addMetadataToResponse);
